                     ...
                     prior=list())
{
  ...
  names(APM)<-names(X)
  names(UVPM.l)<-rownames(THETAPM)<-rownames(UPM)
  rownames(ALPHAPM)<-colnames(Y)
  
  
  fit <- list(BETAPM=BETAPM,GAMMAPM=GAMMAPM, COV=COV,VC=VC, APM=APM,bPM=bPM,U=U,UVPM.l=UVPM.l,THETAPM = THETAPM,UPM=UPM, EFlPM=EFlPM,ETPM=ETPM,
